...
import { BarChart3, TrendingUp, Sparkles, Activity, Layers } from 'lucide-react'
...
import { ControlChartsPanel } from '../components/visualization/ControlChartsPanel'
import { PCAPanel } from '../components/visualization/PCAPanel'

export const AnalyticsPage: React.FC = () => {
  const { shots } = useShotStore()
  const { activeRecipe } = useRecipeStore()
  const [selectedView, setSelectedView] = useState<'3d' | 'correlation' | 'optimization' | 'control-charts' | 'pca'>('3d')
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2 mb-6">
          ...
          >
            ...
            <span className="hidden sm:inline">3D Space</span>
          </button>
          <button
          ...
          <button
            onClick={() => setSelectedView('control-charts')}
            className={`py-3 px-4 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 ${
              selectedView === 'control-charts'
                ? 'bg-gradient-espresso text-white shadow-lg'
                : 'bg-white/10 text-white/60 hover:bg-white/20'
            }`}
          >
            <Activity className="w-4 h-4" />
            <span className="hidden sm:inline">SPC Charts</span>
          </button>
          <button
        ...
        {shots.length === 0 ? (
          ...
          <div>
            {/* Full-width views (Control Charts and PCA) */}
            {(selectedView === 'control-charts' || selectedView === 'pca') ? (
              <motion.div
                key={selectedView}
              ...
              >
                {selectedView === 'control-charts' && <ControlChartsPanel shots={shots} />}
                {selectedView === 'pca' && <PCAPanel shots={shots} />}
              </motion.div>
            ) : (
              /* Grid layout for 3D, correlation, and optimization */
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mt-8">
          <GlassCard className="text-center">
            ...
            <h3 className="font-bold text-white mb-2">ML Predictions</h3>
          ...
          <GlassCard className="text-center">
            ...
            <h3 className="font-bold text-white mb-2">SPC Charts</h3>
            <p className="text-sm text-white/60">
              Pharma-grade process control
            </p>
          </GlassCard>

          <GlassCard className="text-center">
            <div className="bg-pink-500/20 rounded-full p-4 w-16 h-16 mx-auto mb-3 flex items-center justify-center">
              <Layers className="w-8 h-8 text-pink-400" />
            </div>
            <h3 className="font-bold text-white mb-2">PCA Analysis</h3>
            <p className="text-sm text-white/60">
